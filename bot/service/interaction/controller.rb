      ...
      def block_actions(bot_request)
        bot_request.data['actions'].each do |action|
          ...
          when 'unfavourite'
            ...
            Topic::Sns.broadcast(topic: :users, request: bot_request)
          when 'next-recipes' 
            bot_request.events << ::Request::Events::Recipes.search_requested(source: :interaction, 
          ...
          when 'edit-account'
            bot_request.events << ::Request::Events::Users.account_edit_requested(source: :interactions) 
            Topic::Sns.broadcast(topic: :users, request: bot_request)
          end
        end
      end
